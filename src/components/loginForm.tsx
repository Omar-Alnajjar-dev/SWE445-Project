  ...
  return (
    <form className="flex flex-col gap-3 m-5 justify-center items-center">
      {!state?.otp ? (<><h1 className="text-md text-stone-600">Please Enter Your Credintials</h1>
        <input className="w-full border border-gray-400 rounded-md h-10 p-2" type="text" name="username" required placeholder="Username" pattern="!/^[a-zA-Z0-9]+$/" onChange={(e) => { setUsername(e.target.value) }} />
        <input className="w-full border border-gray-400 rounded-md h-10 p-2" type="password" name="password" required placeholder="Password" pattern="" onChange={(e) => { setPassword(e.target.value) }} />
