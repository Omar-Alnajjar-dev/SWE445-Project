...
const { getAllUsers, getUser, isAdmin, getAllPatients, getAllUserswithRoles, getByID, checkUsers, insertUser, getAllRoles, updateRole, updateUser, checkUserExistBefore } = require('@/dbService');
...
export const modifyInfo = async (
  ...
  // CHECK IF USER EXISTS
  const isUserExists = await checkUserExistBefore(formUsername, formEmail, formPersonId);
  // const prevUserData = await getUserByID(parseInt(formPersonId));
  ...
  // INSERT USER INTO DB
  const insert = await updateUser(
    parseInt(formPersonId),
    formFName,
    formMName,
    formLName,
    formUsername,
  ...
  redirect("/adminPages/updatePatients");
